...
<form method="POST" action="{{ route('profile.vendor.product.delivery.new', $physicalProduct -> product) }}">
    ...
    <div class="form-row">
        ...
        <div class="col-md-4 my-2">
            <input type="number" step=".01" min="0" class="form-control @error('price', $errors) is-invalid @enderror" value="{{ old('price') }}" name="price" placeholder="Price in {{ \App\Marketplace\Utility\CurrencyConverter::getLocalSymbol() }}(Ex. 15.99)">
            @error('price', $errors)
            <div class="invalid-feedback">
                {{ $errors -> first('price') }}
            </div>
