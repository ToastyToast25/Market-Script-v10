                    ...
                    @foreach($allCategories as $category)
                                ...
                                @if($category -> children -> isNotEmpty())
                                    @foreach($category->children as $child)
                                        <option value="{{$child->id}}" @if($child -> id == optional($basicProduct) -> category_id) selected @endif > :- {{$child->name}}</option>
                                        @if($child -> children -> isNotEmpty())
                                            @foreach($child->children as $subChild)
                                                <option value="{{$subChild->id}}" @if($subChild -> id == optional($basicProduct) -> category_id) selected @endif> :: {{$subChild->name}}</option>
                                            @endforeach
                                        @endif
                                    @endforeach
                                @endif
