...
class Vendor extends User {
    ...
    public function sales() {
        return $this->hasMany(\App\Models\Purchase::class, 'vendor_id', 'id')->orderByDesc('created_at');
    }
    ...
    public function feedback() {
        return $this->hasMany(\App\Models\Feedback::class, 'vendor_id', 'id');
    }
    ...
    public function getProfileBg() {
       // If the vendor's profile background is not set
    if ($this->profilebg === null) {
        // Get the profile backgrounds from the configuration
        $profileBgs = config('vendor.profile_bgs');

        // Select a random profile background key
        $randomKey = array_rand($profileBgs);

        // Set the profile background property to the selected class
        $this->profilebg = $profileBgs[$randomKey];

        // Save the vendor to persist the selected profile background
        $this->save();
    }

    // Return the profile background class
